...
class BasicModel:
	...
	def _define_inputs(self):
		...
		self.current_epoch = tf.placeholder(tf.int32, shape=[], name='current_epoch')
	...
	def pure_train(self):
		...
		for epoch in range(1, n_epochs + 1):
			...
			for i in range(num_examples // batch_size):
				...
				fetches = [self.train_step, self.accuracy]
				_,acc=self.sess.run(fetches, feed_dict=feed_dict)
			time_per_epoch = time.time() - start_time
			print(acc)
	...
	def conv2d(_input, out_features, kernel_size, strides=1, padding='SAME', param_initializer=None):
		...
		'''if kernel_size>1:
			depthwise_kernel = BasicModel.weight_variable(
				[kernel_size, kernel_size, in_features, 1],
				name='depth_kernel',
				initializer=param_initializer.get('kernel',tf.contrib.layers.xavier_initializer()) #tf.contrib.layers.variance_scaling_initializer())
			)
			pointwise_kernel = BasicModel.weight_variable(
				[1, 1, in_features, out_features],
				name='poiny_kernel',
				initializer=param_initializer.get('kernel',tf.contrib.layers.xavier_initializer()) #tf.contrib.layers.variance_scaling_initializer())
			)
			output = tf.nn.separable_conv2d(input=_input, depthwise_filter=depthwise_kernel, pointwise_filter=pointwise_kernel, strides=[1,1,1,1], rate=[1,1], padding='SAME')
			return output
		else:'''
		kernel = BasicModel.weight_variable(
			[kernel_size, kernel_size, in_features, out_features],
			name='kernel',
			initializer= tf.contrib.layers.variance_scaling_initializer())#param_initializer.get('kernel',tf.contrib.layers.xavier_initializer())
			
		output = tf.nn.conv2d(_input, kernel, [1, strides, strides, 1], padding)
		return output
		
	@staticmethod
	def fc_layer(_input, out_units, use_bias=True, param_initializer=None):
	...
	def flatten(_input):
		...
		output=tf.reshape(_input, [-1, np.prod(input_shape[1:])])
		return output
		'''input=[]
		for tensor in _input:
			if tensor is not None:
				input.append(tensor)
		if len(input)>1:
			_input=tf.concat(input,3)
		input_shape = _input.shape.as_list()
...
class SimpleConvnet(BasicModel):
	def _build_graph(self, only_forward=False):
		...
		if not only_forward:
			...
			print("weight_decay",self.net_config.weight_decay)
